/*
 SeaJS v2.0.0b2 | seajs.org/LICENSE.md
*/
...
var q=u.seajs;if(!q||q.args){var e=u.seajs={version:"2.0.0b2"},xa=H("Object"),A=Array.isArray||H("Array"),C=H("Function"),Y=e.log=function(a,c){u.console&&(c||k.debug)&&console[c||(c="log")]&&console[c](a)},t=e.events={};e.on=function(a,c){if(!c)return e;(t[a]||(t[a]=[])).push(c);return e};e.off=function(a,c){if(!a&&!c)return e.events=t={},e;var b=t[a];if(b)if(c)for(var d=b.length-1;0<=d;d--)b[d]===c&&b.splice(d,1);else delete t[a];return e};var v=e.emit=function(a,c){var b=t[a],d;if(b)for(b=b.slice();d=
b.shift();)d(c);return e},ga=/[^?]*(?=\/.*$)/,ha=/\/\.\//g,P=/\/[^/]+\/\.\.\//g,ia=/([^:\/])\/\/+/g,ka=/\?|\.(?:css|js)$|\/$/,ja=/#$/,qa=/{([^{}]+)}/g,oa=/^(.*?:\/\/.*?)(?:\/|$)/,la=/(?:^|:)\/\/./,ma=/^\.{1,2}\//,na=/^\//,pa=/^[^./][^:]*$/,s=document,m=location,w=I(m.href),f;if(!(f=s.getElementById("seajsnode")))f=s.getElementsByTagName("script"),f=f[f.length-1];var ea=I(f.hasAttribute?f.src:f.getAttribute("src",4))||w;e.cwd=function(a){return a?w=O(a+"/"):w};var x=s.getElementsByTagName("head")[0]||
